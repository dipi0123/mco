...
>
> **2026-07-12 정합 감사 반영(일관성·정직 라벨링만, 기술 방향 불변):** §3 티어1 지연=차용값·Supabase 리전 명시·게이트 승격(발견 10) / §4 PG 큐 우선순위 레인(발견 11) / §5 '무료 티어 성립'에 핀 스왑 원가 헤지(발견 16) / §8 호출률 텔레메트리=recall-health 전용 펜싱(발견 19) / §9 티어1 지연 게이트 행 추가.
...
지연 예산의 근거는 업계 수렴이다: Zep P95<200ms, AgentCore ~200ms, Supermemory 500ms 예산 — 그래서 쿼리 시점 LLM 금지가 원칙이 된다. 쓰기(추출·통합)는 백그라운드 초~수십 초가 현업 정상 범위. 플랫폼 제약(도구 응답 300s 타임아웃, 150k자 결과 캡)은 여유 있게 통과한다. *정합 감사(발견 10): 50–200ms는 타사 차용값이고 Hyperdrive는 커넥션 셋업(~625ms)만 없앨 뿐 쿼리당 지리 RTT는 못 줄인다 — 1차 시장(한국) 기준 Supabase 리전을 서울/도쿄로 못박거나 엣지 리드 리플리카를 전제하고, 티어1 팩-읽기 p95(실 리전·RLS·풀링 포함)를 §9 게이트로 승격한다(trigram 1일차 검증처럼).*
...
**큐도 Postgres.** 별도 큐 인프라(SQS·Redis) 없이 PG 큐로 백그라운드 파이프라인을 돌린다(Honcho deriver 패턴) — 2~4인 팀의 운영 부담 최소화. *정합 감사(발견 11): 웜스타트 대량 배치·수면 사이클·실시간 undo write-back이 한 큐를 공유하므로, 우선순위 컬럼/부분 인덱스 레인으로 실시간과 배치를 분리하고 배치는 배치 API 레인에 고정한다 — 동시 온보딩 시 head-of-line 블로킹·티어1 읽기 co-tenancy 방지. 규모 시 티어1 읽기용 리드 리플리카는 §7 이주 경로.*
...
결론: **무료 티어가 인프라적으로 성립한다**(웜스타트 깊이 캡 전제 — 단 추출 모델 Flash-Lite '핀'은 한국어 벤치 조건부라, 스왑(→GPT-5-mini 등 상위 티어) 시 추출 원가가 상향될 수 있고 무료 티어 성립은 **벤치 통과를 전제**로 한다 — 발견 16). 이 표는 "이 구성으로 서비스가 경제적으로 돌아가는가"에 대한 답이지 가격표가 아니다 — pricing은 상품성 결론(정밀도 실험 통과) 뒤에만 논한다. 참고로 시장의 과금 미터 사실 기록: mem0 = 월 add/retrieval 건수, Supermemory = 중복 제거 토큰, GB 저장 미터는 기각됨(매출∝저장량이면 노이즈 축적을 최적화하게 되어 품질 논거와 자충 — 사업성 분석 §6).

## 6. 보안

**전제 — 구조 리스크 (통제 6항 정본: `.claude/memory/mco-security-model.md`).** MCO는 [lethal trifecta](https://simonwillison.net/2025/Jun/16/the-lethal-trifecta/)(① 비공개 데이터 접근 ② 비신뢰 콘텐츠 노출 ③ 외부 통신)의 세 변을 제품 구조로 전부 갖는다 — 최고 민감의 지속 메모리를 보유하고(①), write-back이 비신뢰 웹·문서·도구 출력 유래의 런 산출물을 기억으로 흡수하며(②), 그 기억이 외부 통신 가능한 임의 에이전트에 주입된다(③). 따라서 **방어 본체 = 아키텍처 제약(아래 통제 1·2), 스캔·필터 = 보조** — "주입된 입력이 중대 행동을 트리거할 수 없게"가 원칙이다(가드레일 95% 차단 = 앱보안 기준 낙제).
...
- **기억 신뢰 티어링 (통제 1).** provenance에 trust_level(사람 단언 > 검증된 런 산출 > 비신뢰 콘텐츠 유래)을 더해, 저신뢰 기억은 고권한 문맥(교차 스코프 브리핑·자격증명 인접 작업)에 자동 주입 금지. 강제 지점 = 기존 가치 판단 게이트·기억 선택(역할 ①⑤) — 보안이 '저장<선택' 코어 메커니즘 위에 얹히므로 신규 컴포넌트 불요. → 스키마 v0.1 필드.
- **write-back = 중대 행동 (통제 2).** 스코프 X의 런은 스코프 X에만 write, 교차 스코프·교차 테넌트 write는 예외 없이 사람 승인 — RLS의 read 격리와 대칭. 오염 기억이 타 계정·법인·프로젝트로 전파되는 confused-deputy 경로 봉쇄.
- **주입 전 스캔 (통제 3 — 보조).** 기억이 모델에 주입되기 전 프롬프트 인젝션·크리덴셜 패턴을 스캔한다(Hermes에서 차용한 관행). 단, 방어 본체가 아니라 보조층이다.
- **대시보드 = IAP 게이트 (통제 4).** 예외 인박스·감사·권한 대시보드에 인증을 자체 빌드하지 않는다('인증은 빌드하지 않는다' 원칙의 연장) — [identity-aware proxy](https://www.pomerium.com/blog/secure-access-for-mcp)(Cloudflare Access류/Pomerium) + SSO로 앞단 차단. → §9 #5 스택 결정의 입력.
- **신뢰 헤더 규율 (통제 5).** forwarded 계열 헤더(CF-Connecting-IP 등)는 엣지가 세팅한 것만 신뢰하고, 클라이언트 제공 신원 헤더는 경계에서 스트립한다(신원 헤더 스푸핑 = 인증 전체 무력화 — trusted-proxy 패턴의 표준 함정). → 구현 1일차 체크.
...
- **암호화.** 전송(TLS)·저장 시 암호화는 관리형 스택(Workers·Supabase) 기본을 적용한다. 기억은 최고 민감 데이터 — 아키텍처 통제 6항(위)은 선확정, **잔여 미결 = 암호화 수준·삭제권 보증·보존 정책**(구현 전 확정, §9 #6).
...
| claude.ai 연결 무음 실패 악명 | **전 레이어 로깅 1일차부터** + 연결 직후 도구 발화 self-test — self-test·온보딩·OAuth 경로는 **제로 상태(신규 미연결) 계정으로 검증**(통제 6: 기연결 테스트는 미페어링 경로 버그를 가림) | 1일차 |
| 웹 표면 호출 의존 (모델이 도구 호출을 결정 — 최대 UX 리스크) | 3중 완화: ⓐ "작업 시작 전 memory_brief" 지침 스니펫 원클릭 설치(온보딩 기본값) ⓑ 대시보드에 앱별 **호출률 텔레메트리**(침묵 열화를 가시 열화로 — ChatGPT 2025-10 커넥터 회귀 전례의 조기 경보; **recall-health 전용 — 런 실행·스팬·비용 추적으로 확장 금지, 관측 제품 아님 — 발견 19**) ⓒ 첫 5분은 우리가 제어하는 호출로 구성 | 1일차~상시 |
...
| 2 | 게이트·선택 정밀도 실험 | **관문 (pre-build 미검증 코어 1개 — 수요·행동·WTP는 post-ship 검증, 아키텍처 §0)** | 베이스라인 = full-context와 BM25 둘 다, 킬 기준 +10점, 기권(abstain) 클래스 포함, 한국어 슬라이스 별도 채점, 점수는 토큰/쿼리·p95와 쌍으로 보고 |
| 3 | 웹 표면 호출률 A/B | 실험 대기 | 지침 스니펫 유/무 비교 — 대시보드 recall-rate 지표의 기준선 |
| 4 | 한글 trigram 1일차 검증 | 배포 시 절차 | `show_trgm('공원')` 비어 있지 않음 확인 |
| 5 | 대시보드 스택 확정 | 미결 | Supabase auth 공유 시 저비용 — 잠정 방향. **신규 입력(통제 4): 인증 자체 빌드 금지, IAP 앞단 게이트 전제로 결정** |
| 6 | 상세 프라이버시·보안 모델 | **부분 선확정** — 아키텍처 통제 6항 확정(§6, 정본 `.claude/memory/mco-security-model.md`); 잔여 미결 = 암호화 수준·삭제권 보증·보존 정책 (차터 §9) | 잔여분 구현 전 확정 |
| 7 | zerank-2 상업 라이선스 협의 | 선택 항목 | 비상업 라이선스 — 필요 시에만 |
| 8 | 차터 v0.3 반영 · 첫 5분 화면 수준 설계 | 후속 세션 | — |
| 9 | 티어1 팩-읽기 지연(실 리전) — 발견 10 | **게이트 추가** | Workers→Hyperdrive→RLS p95 계측, Supabase 리전=1차 시장(서울/도쿄) 정렬 확인 |
...
*2026-07-12 개정: 보안 통제 6항 반영 — §6(전제 + 통제 1~5), §8(제로 상태 검증 조건 = 통제 6), §9 #5(IAP 입력)·#6(부분 선확정). 정본: `.claude/memory/mco-security-model.md`.*
