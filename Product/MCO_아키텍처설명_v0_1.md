...
>
> **2026-07-12 정합 감사 반영(일관성·정직 라벨링만, 새 설계 없음):** §3 `confidence`·`valid_at` '매달린 필드' 표기 / §5 무영향 반영 채점(아키텍처 §8-5 측정 항목) 메모 / §8 모델표 '잠정 기본값' 읽기(발견 16) / §9 브리핑 팩 파생 인덱스 예외(발견 14).
...
필드가 곧 제품 논리다: `scope`가 오염 페인의 해법(벽), `provenance`가 감사·신뢰의 해법("왜 이걸 알아?"에 항상 답할 수 있음), `trust_level`이 주입 오염 방어의 해법(비신뢰 유래 기억은 고권한 문맥에 오르지 못한다 — lethal trifecta 대응, 통제 6항 정본 `.claude/memory/mco-security-model.md`), `ttl_class`+bi-temporal이 신선도의 해법(파괴적 삭제 대신 무효화 마킹 — undo가 무비용인 이유), `gen_queries`가 회수율을 쓰기 시점에 미리 사두는 장치다. *정합 감사 메모: `relations`(모순 동시-인출 방지)·`status`(충돌/갱신)는 소비처가 명확하나, `confidence`와 bi-temporal의 `valid_at`은 현재 쓰기(갱신)만 정의되고 읽는 컴포넌트가 명시되지 않은 '매달린 필드'다 — 선택⑤이 이를 소비하도록 명문화하거나 스키마 v0.1에서 드롭할지는 §8 정밀도 실험의 기여도로 결정한다(린 스키마 원칙).*
...
**기록-후-되돌리기 원칙**(검증 P2로 역전된 설계): 제안→승인이 아니라 자동 커밋+즉시 undo가 기본이다. 승인 피로("아침에 73건 = 반사적 승인")와 승인형 기능의 실제 제거 사례가 근거. ADD-only+invalid_at 스키마 덕에 undo는 무비용이고, 결정 인박스는 모든 쓰기의 관문이 아니라 **예외 처리함**이다 — 방치해도 시스템은 기본값으로 굴러간다. 교차 스코프 write 승인은 UX 규칙이자 보안 통제이기도 하다 — 런은 자기 스코프에만 write한다(write-back = 중대 행동: 오염 기억의 타 스코프 전파 봉쇄, 보안 통제 2). *정합 감사 메모: write-back이 런 배기가스를 흡수하는 이 경로에 '주입한 기억이 행동에 반영됐는지'(페인 2위 무영향)를 채점하는 **'반영 채점 슬라이스'(아키텍처 §8-5 측정 항목)**를 얹는다 — 관측 제품이 아니라 기억 유용성 채점이라 '관측 아님' 경계 내. 반영도 기반 능동 개입 런타임 컴포넌트로의 승격은 실험 결과에 따른 별도 결정.*
...
전 항목에 **2중 게이트**가 적용된 결과다: ⓐ 관리형 호환성(우리 관리형 프로바이더에 실존하는가) ⓑ 한국어 실측 근거(없으면 확정 금지, 자체 벤치 게이트 뒤로). *정합 감사(발견 16): '확정 기본값' 열은 '현 시점 잠정 기본값(벤치3 통과 조건부)'으로 읽어야 한다 — 특히 추출 LLM '핀'은 한국어 공개 벤치 부재라 스왑 시 원가가 이동할 수 있다.*
...
따름 정리: 모델 교체·병행이 자유롭다(락인 없음). 임베딩·검색 스택을 갈아끼워도 파생 인덱스만 재생성하면 된다(§2). *정합 감사(발견 14): 단 브리핑 팩은 예외 — 팩 멤버십이 정본에 없는 누적 사용 상태(인출 빈도)에 의존하므로, 티어 멤버십·인출 빈도를 정본 필드로 올리거나 팩을 '규칙 캐시'로 재분류해야 이 따름정리 범위에 든다.* 데모 목표도 이 원리의 시연이다 — 같은 질문을 Claude 웹·Claude Code·Hermes+MCO에서 던져 같은 브리핑·같은 결론을 얻는 것("원 브레인, 멀티 서피스").
